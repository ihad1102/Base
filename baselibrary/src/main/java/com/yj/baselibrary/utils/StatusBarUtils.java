...
//        }
    ...
    public static void setStyle(Activity acitivty) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            acitivty.getWindow().addFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
            ViewGroup rootView = (ViewGroup) ((ViewGroup)acitivty.findViewById(android.R.id.content)).getChildAt(0);
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            acitivty.getWindow().getDecorView().setSystemUiVisibility(View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN| View.SYSTEM_UI_FLAG_LAYOUT_STABLE);
            acitivty.getWindow().addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
            acitivty.getWindow().setStatusBarColor(Color.TRANSPARENT);
        }
    }

    public static void setStyle(Activity acitivty,int color) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            acitivty.getWindow().addFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
            ViewGroup rootView = (ViewGroup) ((ViewGroup)acitivty.findViewById(android.R.id.content)).getChildAt(0);
            rootView.setFitsSystemWindows(false);
