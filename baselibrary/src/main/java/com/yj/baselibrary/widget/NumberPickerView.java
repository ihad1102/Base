...
public class NumberPickerView extends View {
    // default text color of not selected item  默认不选中项的文本颜色
    private static final int DEFAULT_TEXT_COLOR_NORMAL = 0XFF333333;

    // default text color of selected item  默认选中条目的文本颜色
    private static final int DEFAULT_TEXT_COLOR_SELECTED = 0XFFF56313;

    // default text size of normal item 正常项目的默认文本大小
    private static final int DEFAULT_TEXT_SIZE_NORMAL_SP = 14;

    // default text size of selected item 默认文本大小的选择项
    private static final int DEFAULT_TEXT_SIZE_SELECTED_SP = 16;

    // default text size of hint text, the middle item's right text 默认提示文本的文本大小,中间项的文本
    private static final int DEFAULT_TEXT_SIZE_HINT_SP = 14;

    // distance between selected text and hint text  选中的文本和提示文本之间的距离
    private static final int DEFAULT_MARGIN_START_OF_HINT_DP = 8;

    // distance between hint text and right of this view, used in wrap_content mode  距离提示文本和这个视图,用于包装内容模式
    ...
    // default divider's margin to the left & right of this view 默认的分频器的边缘的左和右视图
    private static final int DEFAULT_DIVIDER_MARGIN_HORIZONTAL = 0;

    // default shown items' count, now we display 3 items, the 2nd one is selected  默认显示项目的统计,现在我们显示3项,第二选择
//    private static final int DEFAULT_SHOW_COUNT = 3;
    private static final int DEFAULT_SHOW_COUNT = 3;
    // default items' horizontal padding, left padding and right padding are both 5dp, 默认项的空白,左填充和右填充都是5 dp,
    // only used in wrap_content mode  只用于包装内容模式
    private static final int DEFAULT_ITEM_PADDING_DP_H = 5;

    // default items' vertical padding, top padding and bottom padding are both 2dp, 默认项的垂直填充,填充和底部填充都是2 dp,
    // only used in wrap_content mode 只用于包装内容模式,
    private static final int DEFAULT_ITEM_PADDING_DP_V = 2;

    // message's what argument to refresh current state, used by mHandler  消息的参数刷新当前状态,由mHandler使用
    private static final int HANDLER_WHAT_REFRESH = 1;

    // message's what argument to respond value changed event, used by mHandler 消息的参数值变化事件作出回应,mHandler使用
    private static final int HANDLER_WHAT_LISTENER_VALUE_CHANGED = 2;

    // message's what argument to request layout, used by mHandlerInMainThread 消息的参数请求布局,由mHandlerInMainThread使用
    private static final int HANDLER_WHAT_REQUEST_LAYOUT = 3;

    // interval time to scroll the distance of one item's height 间隔时间滚动的距离一个条目的高度
    private static final int HANDLER_INTERVAL_REFRESH = 32;//millisecond

    // in millisecond unit, default duration of scrolling an item' distance 在毫秒单位,默认滚动一个项目的时间距离
    private static final int DEFAULT_INTERVAL_REVISE_DURATION = 300;

    // max and min durations when scrolling from one value to another 最大和最小时间当滚动从一个值到另一个地方
    ...
    //compat for android.widget.NumberPicker 兼容的android.widget.NumberPicker
    ...
    //friction used by scroller when fling  摩擦所使用的照片卷轴扔的时候
    ...
    //true to show the two dividers 真正的两个分规
    ...
    // compatible for NumberPicker 兼容的NumberPicker
